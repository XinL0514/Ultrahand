...
import { pictureToVideoScenario, pictureToVideoTextToImageScenario } from '@e2e/testdata/scenarios/classroom';
...
test('可以图生视频', { tag: '@smoke' }, async ({ aiAct, aiTap, aiInput, aiWaitFor, aiAssert, endClassGuard }) => {
  ...
  await aiInput(pictureToVideoTextToImageScenario.prompt, '聊天输入框', { mode: 'append' });
  await aiTap('输入框右侧的纸飞机发送按钮');
 
  const textToImageCondition = `右侧对话流中刚刚发送的“/文生图 ${pictureToVideoTextToImageScenario.prompt}”已经完成：对应的最终图片已经真实渲染出来并且清晰可见，不再显示排队、生成中、加载中、空白图片或进度百分比；不要根据左侧已有图片判断完成`;
  ...
  await openAiPanelOption({ aiTap, aiAct, aiWaitFor }, '图生视频');
  await aiInput(pictureToVideoScenario.prompt, '聊天输入框', { mode: 'append' });
  await aiTap('输入框右侧的纸飞机发送按钮');


  const imageToImageCondition = `只检查右侧对话流中最新的“/图生视频 ${pictureToVideoScenario.prompt}”消息卡片，左侧课件、上方或下方历史消息一律忽略。
