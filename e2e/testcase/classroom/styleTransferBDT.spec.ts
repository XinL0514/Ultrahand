...
test('可以风格转化', { tag: '@smoke' }, async ({ aiAct, aiTap, aiInput, aiWaitFor, aiAssert, endClassGuard }) => {
  await enterFirstClassroom({ aiTap, aiWaitFor, aiAct });
  await openAiPanelOption({ aiTap, aiAct, aiWaitFor }, '文生图');
  await aiInput(styleTransferTextToImageScenario.prompt, '聊天输入框', { mode: 'append' });
  ...
  await openAiPanelOption({ aiTap, aiAct, aiWaitFor }, '风格转化');
  await aiInput(styleTransferScenario.prompt, '聊天输入框', { mode: 'append' });
  await aiTap('输入框右侧的纸飞机发送按钮');
