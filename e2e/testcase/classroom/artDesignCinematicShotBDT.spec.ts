...
test('可以艺术设计大片', { tag: '@smoke' }, async ({ page, aiAct, aiTap, aiInput, aiWaitFor, aiAssert, endClassGuard }) => {
  ...
  await runClassroomGenerationFlow(
    ...
    [
      {
        optionLabel: '文生图',
        prompt: artDesignCinematicShotTextToImageScenario.prompt,
        completionText: textToImageCondition,
      },
      {
        quotePreviousImage: true,
        optionLabel: 'AI艺术大片',
        prompt: artDesignCinematicShotScenario.prompt,
