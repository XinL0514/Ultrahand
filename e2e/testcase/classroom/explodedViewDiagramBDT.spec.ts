...
test('可以解构图', { tag: '@smoke' }, async ({ aiAct, aiTap, aiInput, aiWaitFor, aiAssert, endClassGuard }) => {
  await enterFirstClassroom({ aiTap, aiWaitFor, aiAct });
  await openAiPanelOption({ aiTap, aiAct, aiWaitFor }, '文生图');
  await aiInput(explodedViewDiagramTextToImageScenario.prompt, '聊天输入框', { mode: 'append' });
  ...
  await waitForStableThenAssert(
    { aiWaitFor, aiAssert },
    explodedViewTextToImageCondition,
    explodedViewTextToImageCondition,
    { timeoutMs: 180000, checkIntervalMs: 3000 },
  );
  await aiTap('点击生成图片下方的 一对蓝色双引号 按钮')
  await openAiPanelOption({ aiTap, aiAct, aiWaitFor }, '解构图');
  await aiInput(explodedViewDiagramScenario.prompt, '聊天输入框', { mode: 'append' });
  await aiTap('输入框右侧的纸飞机发送按钮');
  ...
  await waitForStableThenAssert(
    { aiWaitFor, aiAssert },
    explodedViewCondition,
    explodedViewCondition,
    { timeoutMs: 360_000, checkIntervalMs: 3000 },
  );
