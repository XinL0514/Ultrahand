...
test('可以参考生视频', { tag: '@smoke' }, async ({ page, aiAct, aiTap, aiInput, aiWaitFor, aiAssert, endClassGuard }) => {
  ...
  await runClassroomGenerationFlow(
    ...
    [
      {
        optionLabel: '文生图',
        prompt: referenceToVideoTextToImageScenario.prompt,
        completionText: textToImageCondition,
      },
      {
        quotePreviousImage: true,
        optionLabel: '参考生视频',
        prompt: referenceToVideoScenario.prompt,
