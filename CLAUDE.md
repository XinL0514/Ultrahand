...
`npm test` first runs `e2e/globalSetup.ts` once: it performs a real AI-driven login and saves the session to `e2e/.auth/user.json`, which all specs then reuse via `storageState` (see `playwright.config.ts`) so tests don't each re-login. Delete that file to force a fresh login.
...
- `e2e/globalSetup.ts` — logs in once per account in the pool via a `PlaywrightAgent` (Midscene) driving natural-language steps, persisting one `storageState` file per account slot (`e2e/.auth/user-<slot>.json`).
...
- Login form copy / presence of captcha, etc. is only known from a real run of `globalSetup.ts`. If login starts failing, first adjust the natural-language steps there rather than the underlying architecture.
